const String BASE_URL = "https://dzor.in/api/";
const String PRODUCT_PHOTO_BASE_URL = "${BASE_URL}photos/products";
const String PROMOTION_PHOTO_BASE_URL = "${BASE_URL}photos/promotions";
const String CATEGORY_PHOTO_BASE_URL = "${BASE_URL}photos/subcategories";
const String SELLER_PHOTO_BASE_URL = "${BASE_URL}photos/accounts";
const String SELLER_PROFILE_PHOTO_BASE_URL = "${BASE_URL}photos/sellers";
const String APPOINTMENT_URL = "http://appointment.dzor.in/api/appointments/";
